...
class EvenementenController extends Controller
{
    ...
    public function get_add_evenementen()
    {
        ...
        return view('admin.evenementen.add', [
            'urls' => $urls,
        ]);
    }
    ...
    public function post_add_evenementen(Request $request)
    {
        ...
        if ($add) {
            ...
            return redirect()->route('admin.evenementen');
        } else {
    ...
    public function get_edit_evenementen($id_encrypted)
    {
        ...
        if (is_object($evenement)) {
            return view('admin.evenementen.edit', [
                'evenement' => $evenement,
                'urls' => $urls,
            ]);
        } else {
    ...
    public function post_edit_evenementen(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'prijs' => 'required',
            'snelle_info' => 'required',
            'url' => $request->has_static_page === 'on' ? 'required' : 'required|unique:evenementen,url,'.$request->id,
            'start' => 'required',
            'eind' => 'required',
            'banner_color' => 'required_if:has_static_page,off',
            'banner_pattern' => 'required_if:has_static_page,off',
